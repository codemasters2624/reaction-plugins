 ...
 * @summary clones a door variant into a new variant
 ...
 */
export default async function cloneDoorVariants(context, input) {
  ...
  const newVariants = await Promise.all(
    variantIds.map(async (variantId) => {
      const existingVariant = await Doors.findOne({ _id: variantId });
      const doorId = existingVariant.ancestors[0];
      ...
      await Promise.all(
        sortedVariants.map(async (sortedVariant) => {
          ...
          const { insertedId: clonedVariantId } = await Doors.insertOne(
            clonedVariantObject,
            { validate: false }
          );

          if (!clonedVariantId) {
            Logger.error(`cloneDoorVariants: cloning of ${variantId} failed`);
      ...
      const newFinalDoor = await Doors.findOne({ _id: variantNewId });
